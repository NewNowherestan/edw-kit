...
.PHONY: help bootstrap shell terminal workstation full doctor reload lock secrets macos-defaults check
...
check: ## Syntax-check all shell scripts (what CI runs)
	@for f in bootstrap.sh install.sh lib/*.sh scripts/*.sh; do \
		bash -n "$$f" && echo "✓ $$f"; \
	done
